...
//
//      FILE NAME:  lab3p2_main_template.c
//
//-----------------------------------------------------------------------------
//
// DESCRIPTION:
...
int main(void)
{
  ...
  reg_value = test_reg16;
  if (my_asm_16bitcheck(0x000E, A2_BIT_MASK)) {
  ...
  if (my_asm_16bitcheck(reg_value, MD_BIT_MASK)) {
    ...
    my_asm_16bitclr(reg_value, MODE_01_BIT_VALUE);
  } else {
  ...
  // enter your code here for problem 1
  reg_value = test_reg16;
  my_asm_16bitclr(reg_value, PIE_BIT_MASK);
  my_asm_16bitclr(reg_value, EME_BIT_MASK);
  my_asm_16bitclr(reg_value, RD_BIT_MASK);
  my_asm_16bitclr(reg_value, MD_BIT_MASK);
  my_asm_16bitclr(reg_value, CRS_BIT_MASK);
  my_asm_16bitclr(reg_value, MODE_01_BIT_VALUE);
  my_asm_16bitclr(reg_value, MODE_10_BIT_VALUE);
  my_asm_16bitclr(reg_value, MODE_BIT_MASK);
  my_asm_16bitclr(reg_value, PRS_BIT_MASK);
  my_asm_16bitclr(reg_value, A0_BIT_MASK);
